...
async function fixGW1Picks() {
  try {
    console.log('🏆 Fixing GW1 picks for all players...')
    ...
    // GW1 Picks data - CORRECT picks
    const gw1Picks = [
      { playerName: "Jonah McGowan", team: "Liverpool" },
      { playerName: "Abboud Hammour", team: "Tottenham Hotspur" },
      { playerName: "Chris Grube", team: "Tottenham Hotspur" },
      { playerName: "Max Reid", team: "Tottenham Hotspur" }
    ]
    
    // Add/update picks for each player
    for (const pickData of gw1Picks) {
      ...
      if (existingPick) {
        ...
        console.log(`✅ Updated pick for ${pickData.playerName}: ${pickData.team}`)
      } else {
        // Create new pick
    ...
    console.log('🎉 GW1 picks fixed successfully!')
    ...
    console.error('❌ Error fixing GW1 picks:', error)
  } finally {
