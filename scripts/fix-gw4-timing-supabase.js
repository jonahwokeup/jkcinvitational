...
async function main() {
  console.log('🎯 Fixing GW4 timing and adding fixtures to Supabase production database...');
  ...
  try {
    ...
    // Fix GW4 lock time to September 13th, 7:30 AM ET
    const correctLockTime = new Date('2025-09-13T11:30:00.000Z'); // 7:30 AM ET = 11:30 AM UTC
    
    await prisma.gameweek.update({
      where: { id: gw4.id },
      data: {
        lockTime: correctLockTime
      }
    });
    
    console.log(`✅ Updated GW4 lock time to: ${correctLockTime}`);
    
    // Add all 10 GW4 fixtures
    const fixtures = [
    ...
    console.log('🔒 GW4 lock time corrected to September 13th, 7:30 AM ET');
